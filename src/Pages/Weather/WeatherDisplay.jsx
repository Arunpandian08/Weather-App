import React from 'react';
import './WeatherDisplay.css';
import Search from '../../assets/search.svg';
import { useWeatherContext } from '../../Context/WeatherProvider.jsx';
import Card from '../../Components/Card/Card.jsx';
import Navbar from '../../Components/Navbar/Navbar.jsx';
import Footer from '../../Components/Footer/Footer.jsx';

const WeatherDisplay = () => {
    const {
        countries,
        selectedCountry,
        setSelectedCountry,
        selectedCity,
        setSelectedCity,
        setFlagURL,
        getWeather,
    } = useWeatherContext();

    const handleCountryChange = (e) => {
        const selected = e.target.value;
        setSelectedCountry(selected);
        const selectedCountryData = countries.find(country => country.name === selected);
        if (selectedCountryData) {
            setSelectedCity(selectedCountryData.capital || '');
            setFlagURL(selectedCountryData.flag);
        } else {
            setSelectedCity('');
            setFlagURL('');
        }
    };

    const handleSearch = () => {
        getWeather();
    };

    return (
        <>
            <Navbar />
            <div className="main">
                <div className="row">
                    <div className="col-4 display1">
                        <div className="card1 h-100 p-2">
                            <div className="card-title mt-4">
                                <h3 className="text-center">
                                    <svg
                                        className="weather"
                                        version="1.1"
                                        id="Layer_1"
                                        x="0px"
                                        y="0px"
                                        width="50px"
                                        height="50px"
                                        viewBox="0 0 100 100"
                                        xmlSpace="preserve"
                                    >
                                        <image
                                            id="image0"
                                            width="100"
                                            height="100"
                                            x="0"
                                            y="0"
                                            href="data:image/png;base64,"
                                        ></image>
                                    </svg>{' '}
                                    Weather
                                </h3>
                            </div>
                            <div className="card-body1 mt-5 mb-5 d-flex justify-content-center align-items-center">
                                <label htmlFor="country-select">Choose a Country:</label> &nbsp;&nbsp;
                                <select
                                    className="select d-inline-flex focus-ring py-1 px-2 text-decoration-none border rounded-2"
                                    name="countries"
                                    id="country-select"
                                    value={selectedCountry}
                                    onChange={handleCountryChange}
                                >
                                    <option value="">&nbsp;Please choose an option&nbsp;</option>
                                    {countries.map((country) => (
                                        <option key={country.cca2} value={country.name}>
                                            {country.name}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            <div className="card-body2 mb-5 d-flex justify-content-center align-items-center">
                                <label htmlFor="city-select">Capital City:</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <select
                                    className="select d-inline-flex focus-ring py-1 px-2 text-decoration-none border rounded-2"
                                    name="cities"
                                    id="city-select"
                                    value={selectedCity}
                                    onChange={(e) => setSelectedCity(e.target.value)}
                                >
                                    <option value="">&nbsp;Please choose an option&nbsp;</option>
                                    {selectedCountry && countries.find(country => country.name === selectedCountry)?.capital ? (
                                        <option key={selectedCountry} value={selectedCity}>
                                            {selectedCity}
                                        </option>
                                    ) : (
                                        <option value="" disabled>No capital available</option>
                                    )}
                                </select>
                            </div>
                            <div className="search-btn">
                                <button type="button" className="button" onClick={handleSearch}>
                                    <img src={Search} alt="search-svg" />
                                    Search
                                </button>
                            </div>
                        </div>
                    </div>
                    <div className="col-8">
                        <div className="display">
                            <Card />
                        </div>
                    </div>
                </div>
            </div>
            <Footer />
        </>
    );
};

export default WeatherDisplay;
